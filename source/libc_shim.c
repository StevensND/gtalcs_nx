...
// and the SO_* options differently, so translate before forwarding (AF_INET/
// SOCK_* match). Binary uses level 1, optname 2/6/0x4000 among others.
...
// libc++_shared binds std::cout/cerr to &__sF[1]/[2]; these wrappers absorb
// accesses to those fake FILEs and forward everything else to real stdio.
// ---------------------------------------------------------------------------

uint8_t fake_sF[3][0x100]; // referenced by imports.c too

// fake stdout FILE for libGame.so's `stdout` import (std::cout-style writes);
// points into the fake __sF array so the is_fake_file() fakes absorb it
...
// fopen with a large stream buffer for the big archives (.ras/.msf): their
// LZW streams issue many small reads/seeks where fsdev round trips dominate.
...
// ---------------------------------------------------------------------------
// loose-file existence index
//
// The engine probes disk for a loose-file override before every .ras read
// (thousands of probes/boot, each miss a slow SD lookup). data/ and es2/ are
// immutable, so index them once and answer probes from memory; other paths
// (settings.bin etc., created at runtime) always hit the filesystem.
// ---------------------------------------------------------------------------
...
// tight loop forever (black-screen hang at gameplay start), so hand back a
// valid silent MP3 on miss to let the open + mpg123 decode succeed and proceed.
...
// (the game allocates the bionic type; we stash a real-object pointer in its
// first bytes, like the mutex fakes)
// ---------------------------------------------------------------------------
...
// Named POSIX semaphores: return a heap holder pointing at a FakeSem (the same
// shape sem_init_fake leaves) so the unnamed sem_*_fake reuse it. Name ignored;
// LCS only uses these as private in-process semaphores.
